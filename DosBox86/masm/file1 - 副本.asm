...
main:
	...
	;mov filename[9],0				;之前测试的时候用过给定长度的文件名
...
display_by_control:
	...
	int 21h
;	;mov word ptr file_size[2], dx  ;等价以上四句话
...
;	call near ptr Tester
	...
	;mov si, 9	;让读到的字符串有结束标志	;\
	;mov buf[si], '$'				; \	
	;mov dx, offset buf				; /这段代码曾用于测试buf中的内容
	...
	;mov si, 0h					 ; \  
	;mov cx, 75					 ;  /这段代码用于测试函数coordinate_calculating是否正确
...
compare:
	...
	;mov dl, filename[si+1]				;曾用于测试是否已经到达文件名末尾
	;call near ptr Tester
...
show_this_row endp
;-------------------------------------------------------------------------------------------
bytes_in_buf_calculating proc near
	push ebx
	push eax
